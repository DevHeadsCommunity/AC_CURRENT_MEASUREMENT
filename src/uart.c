...
void uart_init(void) {
	...
	RCC->APB2ENR |= 1 << 2;  // enable port A since TX = PA9 and RX = PA10
	RCC->APB2ENR |= 1 << 0;
	...
	//USART2->BRR = 0x1D4C;	//0x271;//0x1D4C; // setting the baudrate to 9600
	 USART2->BRR = 0xEA6;
...
//				}
				debug_flag = 0;
				temp_uart_data[data_debug_position] = (char) USART2->DR;
				data_debug_position++;
